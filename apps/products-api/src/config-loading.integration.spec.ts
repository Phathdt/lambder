...
describe('products-api: config loading', () => {
  test('loadConfig: successfully parses valid environment variables', () => {
    ...
    expect(config.JWT_PUBLIC_KEY_PEM).toBe(validEnv.JWT_PUBLIC_KEY_PEM);
  ...
  test('loadConfig: throws on empty JWT_PUBLIC_KEY_PEM', () => {
    const invalidEnv = {
      DATABASE_URL: 'postgresql://localhost/test',
      REDIS_URL: 'redis://localhost:6379',
      JWT_PUBLIC_KEY_PEM: '',
